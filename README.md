...
5）缓存穿透：查询缓存未命中的KEY，导致请求到了DB；解决方案：1，增加未名字的key到缓存。2使用布隆过滤器  
   缓存击穿：某个热点数据过期，导致大量请求到了DB；解决方案：1，设置Key永不过期。2，使用互斥锁，确保只要一个请求到DB，其余请求挂起，等待数据从DB重新加载到缓存。  
   缓存雪崩：大量缓存，在同一时间内过期导致请求到DB；解决方案：将过期时间进行合理分散，避免大量KEY同一时间过期；
## **7.JAVA代理模式**  
### 1）静态代理：  
...
MySQL ACID实现原理：原子性基于undo log；持久性基于redo log和刷盘；隔离性基于MVCC和锁；  
...
## **11.对volatile的理解**   
1）可见性  
因为在多核CPU环境下，每个CPU有自己的寄存器和缓存，所以每个线程在工作时优先读取和写入的数据均发生在CPU缓存中，也就是JVM内存模型中定义的工作内存，volatile关键字保证了所有线程都在使用vloatile变量时去主内存进行同步，而且必须是在其他线程修改之后才能同步到，因为volatile变量具有可见性。  
2）有序性  
